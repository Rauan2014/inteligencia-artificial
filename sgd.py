...
def sgd(f,
    ...
    for iter in range(start_iter + 1, iterations + 1):
        ...
        loss, gradient = f(x)  # Chamada da função para calcular a perda e o gradiente
        x = x - step * gradient
...
def sanity_check():
    ...
    t1 = sgd(quad, np.array([0.5]), 0.1, 1000, PRINT_EVERY=100)
    print("teste 1:", t1)
    assert np.allclose(t1, np.zeros_like(t1), atol=1e-6)

    t2 = sgd(quad, np.array([0.0]), 0.01, 1000, PRINT_EVERY=100)
    print("teste 2:", t2)
    assert np.allclose(t2, np.zeros_like(t2), atol=1e-6)

    t3 = sgd(quad, np.array([-1.5]), 0.01, 1000, PRINT_EVERY=100)
    print("teste 3:", t3)
    assert np.allclose(t3, np.zeros_like(t3), atol=1e-6)
